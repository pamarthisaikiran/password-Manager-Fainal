...
class Passwords extends Component {
  ...
  /* getFilteredAppointmentsList = () => {
    const {profileDetails, isFilterActive} = this.state

    if (isFilterActive) {
      return profileDetails.filter(
        eachTransaction => eachTransaction.isShow === true,
      )
    }
  ...
  renderProfilesList = () => {
    ...
    // const profileDetailsF = this.getFilteredAppointmentsList()
    let list
  ...
  onCheck = event => {
    const {profileDetails} = this.state
    this.setState(prev => ({
      profileDetails: prev.profileDetails.map(each => {
